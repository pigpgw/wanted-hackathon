...
| 2026-09-19 | DEPLOY | 프로덕션 모델 `gpt-4o-mini` 확정 — 로컬 eval 13케이스 재측정: decompose 13/13·exec 100%·P0 0·분류일치 10/13 (동결 수치와 동일). Vercel CLI 배포 → https://wanted-hackathon.vercel.app , ops_check status=200 latency=0.07s canonical=ok, 직접 입력 decompose 실측 정상 | `scripts/ops_check.sh` |
