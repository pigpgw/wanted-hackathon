...
| 2026-09-19 | BUILD | 첫 방문 온보딩을 react-joyride로 교체 (사용자 명시 요구) — Joyride v3 6스텝 투어: 중앙 환영 → 서술 입력 → 빈도/시간 → 예시 버튼 → 분해 버튼 → 이후 흐름 중앙. data-tour 속성 스포트라이트, 조건부 렌더 요소는 중앙 스텝 대체. sessionStorage 세션 1회 +「사용법」버튼 key-remount 재시작, 토스 블루·한글 locale | `components/automation-app.tsx`, `package.json` (react-joyride ^3.2.0) — tsc·build 통과, 번들 +26kB |
