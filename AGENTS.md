...
- **병렬 세션 소유권**: NEXT.md의 작업 패키지(WP)별 파일 소유권을 따른다. 소유권 밖 파일을 건드려야 하면 작업 전 NEXT.md에 먼저 기록 — 조용한 교차 수정 금지. 세션 간 조율은 파일 기반(NEXT.md·DECISIONS.md) — 명령 주입은 닫힌 세션에만 가능(프로세스 lock). 절차: NEXT.md "오케스트레이션" 섹션.
