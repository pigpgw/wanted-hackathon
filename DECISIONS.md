...
- **비개발자 UX·출력 양식 패스 — 엑셀 입력(TSV)·엑셀/PDF 내보내기·온보딩·진행 안내** (2026-09-19, 사용자 지시 "사용자는 개발자가 아님")
  ...
  - 온보딩: 처음엔 react-joyride 대신 자체 카드를 택했으나 **사용자가 react-joyride 명시 요구로 뒤집음** — Joyride v3 스텝 투어 구현: 첫 방문 자동 시작(sessionStorage 플래그, 탭 닫으면 리셋) + 헤더「사용법」버튼 재시작(key remount로 0번 스텝부터). 조건부 렌더인 2·3단계 요소는 타겟 불가라 화면 중앙 스텝으로 흐름 설명, data-tour 속성으로 실제 요소 스포트라이트. 토스 블루 primaryColor·한글 locale.
