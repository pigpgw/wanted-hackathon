...
- **투어를 "보여주기"→"실제 구동"으로 전환** (2026-09-19, 사용자 지시 "예시데이터가 클릭되면서 joyride가 되야지"): Joyride v3 스텝별 `before` 훅으로 각 단계가 실제 액션 수행 — 예시 채우기→분해→추천 선택→실행까지 투어가 앱을 직접 구동. 최신 상태는 ref(tourApi/tourSnap)로 읽어 stale 클로저 방지, 실행 완료 상태는 가드로 중복 방지, 타겟 미발견 시 투어 종료+에러박스가 대신 안내. 첫 방문 투어 1회당 LLM decompose 1회 비용 발생(실행은 프리셋 캐시라 무료) — 데모 가치 판단으로 수용.
- **TDS 공식 팔레트 전수 대조·통일** (2026-09-19): 커스텀 틴트 10종(`#f5f9ff`·`#d6e8ff`·`#00a661` 등)을 공식 TDS 값(blue50·blue100·green700·green50·orange800·yellow50·red50)으로 교체 — 커스텀 색이 팔레트와 미세하게 달라 "토스인데 어색한" 인상을 만들던 것을 제거.
- **코드-문서 전수 동기화 패스** (2026-09-19, 사용자 지시): 코드 SoT 기준 불일치 일괄 정정 — ①`.tsv` 파일이 `accept=".csv,.tsv,…"`에 있으나 업로드 검증 `["csv","txt","md"]`에서 거부되던 버그 수정(확장자 허용+행 cap) ②`engine.ts` 기본 모델을 `solar-pro3`/Upstage → `gpt-4o-mini`/OpenAI로 변경 — 프로덕션 env·`.env.example` 권장·제출 문서와 정합 (env 미설정 환경도 문서와 같은 기본값).
